...
#define NO_PRINT_BF16
#include "../gemmini-rocc-tests/include/bfloat16.h"
...
#ifdef ELEM_T_IS_FLOAT
              value2 = elem_t_bits_to_elem_t(read_from_dram<elem_t_bits>(dram_byte_addr));
#else
              value2 = read_from_dram<elem_t>(dram_byte_addr);
#endif

#ifdef HAS_MVIN_SCALE
              value2 = mvin_scale(value2, load_scale);
#endif

#ifdef ELEM_T_IS_LOWPREC_FLOAT
            value = bf16_to_f32(value2);
#else
            value = value2;
#endif
            }
...
#ifdef ELEM_T_IS_FLOAT
#ifdef ELEM_T_IS_LOWPREC_FLOAT
            dprintf("%f ", bf16_to_f32(gemmini_state.spad.at(spad_row).at(spad_col)));
#else
...
#ifdef ELEM_T_IS_LOWPREC_FLOAT
          gemmini_state.pe_state.at(i).at(j) = bf16_to_f32(preload_value);
#else
...
#ifdef ELEM_T_IS_LOWPREC_FLOAT
          results.at(i).at(j) += bf16_to_f32(a) * gemmini_state.pe_state.at(k).at(j);
#else
...
#ifdef ELEM_T_IS_LOWPREC_FLOAT
          gemmini_state.pe_state.at(i).at(j) += bf16_to_f32(a) * bf16_to_f32(b);
#else
...
#ifdef ELEM_T_IS_FLOAT
#ifdef ELEM_T_IS_LOWPREC_FLOAT
          dprintf("%f ", bf16_to_f32(gemmini_state.spad.at(base_sp_addr + gemmini_state.c_stride * i).at(j)));
#else
...
#ifdef ELEM_T_IS_LOWPREC_FLOAT
        value += (bf16_to_f32(A->at(i).at(k))) * (bf16_to_f32(B->at(k).at(j)));
#else
...
#ifdef ELEM_T_IS_LOWPREC_FLOAT
    return bf16_less(0, value) ?
#else
    return value > 0 ?
#endif
    static_cast<elem_t>(value) : static_cast<elem_t>(0);
  } else {
...
#ifdef ELEM_T_IS_LOWPREC_FLOAT
  acc_t scaled = MVIN_SCALE_ACC(bf16_to_f32(value), scale);
#else
...
#ifdef ELEM_T_IS_LOWPREC_FLOAT
  acc_t shifted = ROUNDING_RIGHT_SHIFT(bf16_to_f32(value), shift);
#else

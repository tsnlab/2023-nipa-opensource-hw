...
class ChipTop(implicit p: Parameters) extends LazyModule with BindingScope
  ...
  lazy val module: LazyModuleImpLike = new ChipTopImpl(this)
    
}

class ChipTopImpl(l: ChipTop) extends LazyRawModuleImp(l) { 
    val wrapperio = IO(new WrapperIO)
    wrapperio <> l.lazySystem.asInstanceOf[DigitalTop].module.wrapperio
    dontTouch(wrapperio)
}
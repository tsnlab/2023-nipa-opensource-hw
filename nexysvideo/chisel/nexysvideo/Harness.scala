...
class NexysVideoHarness(override implicit val p: Parameters) extends NexysVideoShell with HasHarnessSignalReferences
{
  ...
  println(s"NexysVideo FPGA Base Clock Freq: ${dp(DefaultClockFrequencyKey)} MHz")
  ...
  InModuleBody {
    ...
    other_leds(4) := ddrOverlay.mig.module.io.port.init_calib_complete
